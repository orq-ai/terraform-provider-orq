...
// acceptance harness. The provider block reads ORQ_API_BASE_URL / ORQ_API_KEY from the env.
...
func testAccPreCheck(t *testing.T) {
	t.Helper()
	for _, k := range []string{"ORQ_API_BASE_URL", "ORQ_API_KEY"} {
		if os.Getenv(k) == "" {
...
func TestAccProjectsDataSource(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				// Empty provider block => url/api_key resolve from ORQ_API_BASE_URL/ORQ_API_KEY.
				Config: `
provider "orq" {}
